...
def is_create_task_command(text):
    """检测是否为创建待办指令"""
    return bool(re.match(r'^待办[：:\s]', text))
...
def is_complete_command(text):
    """检测是否为完成任务指令（含别名：完成/销项/搞定/done/已完成）"""
    return bool(re.match(r'^(完成|销项|搞定|done|已完成)[：:\s]', text, re.IGNORECASE))
