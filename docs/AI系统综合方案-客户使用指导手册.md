...
## XI.7 V49 新增 DeepSeek 群指令（场景3）
在总控群 @助手 发送以下指令，DeepSeek 自动处理并写回对应多维表：

| 群指令 | 作用 | 写入表 |
|---|---|---|
| `记录错题：<错题描述>` | 记录错题+AI解析 | 学习卡片表 |
| `生成学习周报：<周次/重点>` | 生成本周学习周报 | 洞察笔记表 |
| `更新用户画像：<本周特征>` | 演化用户画像标签 | 用户画像表 |
| `系统健康诊断：<环境描述>` | 健康诊断+风险评估 | 系统健康表 |
| `问系统：<问题>` | RAG问答，文档未覆盖时AI直接回答 | 无（仅回复） |

> 备注：三报/心跳/备份等定时任务由计划任务自动执行，无需群内触发。
